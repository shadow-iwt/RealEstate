...
async function runTests() {
  ...
  try {
    ...
    const allBelongToUser = leadsRes.data.every((lead: any) => lead.userId === userId);
    ...
    if (allBelongToUser) {
      ...
      const invalidLeads = leadsRes.data.filter((lead: any) => lead.userId !== userId);
    ...
    const propsValid = propsRes.data.every((prop: any) => prop.userId === userId);
